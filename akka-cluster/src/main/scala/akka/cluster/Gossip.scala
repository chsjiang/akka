...
private[cluster] case class Gossip(
  ...
  def leader: Option[Address] = members.find(m ⇒ m.status != Joining && m.status != Exiting && m.status != Down).
    orElse(members.toIndexedSeq.sorted(Member.leaderStatusOrdering).headOption).map(_.address)
